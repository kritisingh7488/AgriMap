...
export default function Edit({ layer }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        is_public: layer.is_public !== undefined ? layer.is_public : true,
        status: layer.status || 'active',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('admin.layers.update', layer._id || layer.id));
    };
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-bold leading-tight text-stone-800 dark:text-[#E8E4DF]">
                    ✏️ Edit GIS Layer: <span className="text-earth-dark dark:text-earth-warm">{layer.name}</span>
                </h2>
            }
        >
            <Head title={`Edit Layer - ${layer.name}`} />
